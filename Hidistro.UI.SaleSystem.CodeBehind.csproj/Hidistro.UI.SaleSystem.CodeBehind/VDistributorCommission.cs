		...
		protected override void AttachChildControls()
        {
			...
			else
			{
				...
				if (userIdDistributors != null && userIdDistributors.UserId > 0)
				{
                    ...
                    Globals.Debuglog("lblsurpluscommission.Money：" + userIdDistributors.ReferralBlance.ToString(), "_Debuglog.txt");
                    //是否需要查询月销售额
                    int MonthPath = 0;
                    ...
                    //Globals.Debuglog("团队奖励返回数据："+ returnMoney, "_Debuglog.txt");
					...
					if (dataTable != null && dataTable.Rows.Count > 0)
					{
                        ...
                        Globals.Debuglog("lblcommission.Money：" + dataTable.Rows[0]["CommTotal"].ToString(), "_Debuglog.txt");
                        this.lblsaleprice.Money = dataTable.Rows[0]["OrderTotal"];
                        Globals.Debuglog("lblsaleprice.Money：" + dataTable.Rows[0]["OrderTotal"].ToString(), "_Debuglog.txt");
                    }
					if (userIdDistributors.DistributorGradeId == DistributorGrade.OneDistributor)
					{
						...
						if (dataTable != null && dataTable.Rows.Count > 0)
						{
                            ...
                            Globals.Debuglog("lbltwocommission.Money：" + dataTable.Rows[0]["CommTotal"].ToString(), "_Debuglog.txt");
                            this.lbltwosaleprice.Money = dataTable.Rows[0]["OrderTotal"];
                            Globals.Debuglog("lbltwosaleprice.Money：" + dataTable.Rows[0]["OrderTotal"].ToString(), "_Debuglog.txt");
                        }
						...
						if (dataTable != null && dataTable.Rows.Count > 0)
						{
                            ...
                            Globals.Debuglog("lblthreecommission.Money：" + dataTable.Rows[0]["CommTotal"].ToString(), "_Debuglog.txt");
                            this.lblthreesaleprice.Money = dataTable.Rows[0]["OrderTotal"];
                            Globals.Debuglog("lblthreesaleprice.Money：" + dataTable.Rows[0]["OrderTotal"].ToString(), "_Debuglog.txt");
                        }
					}
					if (userIdDistributors.DistributorGradeId == DistributorGrade.TowDistributor)
					{
