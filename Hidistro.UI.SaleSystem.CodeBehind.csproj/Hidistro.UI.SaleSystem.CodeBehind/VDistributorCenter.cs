	...
	public class VDistributorCenter : VMemberTemplatedWebControl
	{
		...
		protected override void AttachChildControls()
        {
			...
			else
			{
                ...
                int CheckDate = Convert.ToInt32(Starttime.Day.ToString());
                if (CheckDate >= 15)
                {
                    StartMonth = getYear + "-" + getMonth.ToString() + "-15";
                }
                else
                {
                    StartMonth = getYear + "-" + (getMonth - 1).ToString() + "-15";
                }
                DateTime ss = Convert.ToDateTime(StartMonth);
                //
