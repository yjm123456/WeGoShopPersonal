...
namespace Hidistro.UI.Common.Controls
{
    ...
    public abstract class VshopTemplatedWebControl : TemplatedWebControl
    {
        ...
        public void CheckShopUserState()
        {
            string BusId = ConfigurationManager.AppSettings["BusinessId"].ToString();
            var mamager = ManagerHelper.GetManageByBusId(BusId);
            if (mamager == null)
            {
                this.GotoResourceNotFound("未找到该商户信息");
                return;
            }
            if (mamager.ShopUserState != 1)
            {
                this.GotoResourceNotFound("该商户已过期或已被禁用");
                return;
            }
        }
	}
