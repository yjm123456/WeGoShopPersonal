...
namespace Hidistro.UI.Web.Admin.settings
{
	public class AddManager : AdminPage
	{
		...
		protected void btnSave_Click(object sender, System.EventArgs e)
		{
			...
			ManagerInfo manager = ManagerHelper.CheckAdminName(this.txtUserName.Text.Trim());
			if (manager != null)
			{
				this.ShowMsg("用户名已存在", false);
				return;
			}
            string BusId=ConfigurationManager.AppSettings["BusinessId"].ToString();
            manager = ManagerHelper.GetManageByBusId(BusId);
            if (ManagerHelper.Create(new ManagerInfo
            {
                ...
                DueTime = manager.DueTime,

                CompanyName = manager.CompanyName,
                LiasonManName = manager.LiasonManName,
                Phone = manager.Phone,
                ApplicationRange=manager.ApplicationRange,
                Remark=manager.Remark,
                OrganizingInstitution=manager.OrganizingInstitution,
                InstitutionalType=manager.InstitutionalType,
                AccountName=manager.AccountName,
                DepositBank=manager.DepositBank,
                BankAccount=manager.BankAccount,
                IdentityCard=manager.IdentityCard,
                ContractNo=manager.ContractNo,
                SaleManId=manager.SaleManId,
                SaleManName=manager.SaleManName
            }))
			{
