	...
	public class wx_Pay : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
            try
            {
                ...
                string sql = @" SELECT * FROM [shop2.haiwangcn.cn].[dbo].[Hishop_Orders] WHERE OrderMarking='"+this.OrderId+"'";
                //string sql = @" SELECT * FROM [shop2.haiwangcn.cn].[dbo].[Hishop_Orders] WHERE PayDate is not null and  PayDate > '2019-05-10 16:34:00' AND OrderStatus = 2  ORDER BY OrderDate DESC";
                ...
                string sql2 = @"SELECT ProductId,SKU,Quantity,ItemDescription FROM [shop2.haiwangcn.cn].[dbo].[Hishop_OrderItems] WHERE OrderId =@OrderID";
                ...
                if (Orders.Count > 0 && order.Count > 0)
                {
                    for (int j = 0; j < Orders.Count; j++)
                    {
                        ...
                        //获取产品信息（productCode）
                        string sql3 = @"SELECT ProductCode FROM [shop2.haiwangcn.cn].[dbo].[Hishop_Products] WHERE ProductId=@ProductID";
                        o.ProductCode = connection.GetProductDetails(sql3, Orders[j].ProductId.ToString());
                        //获取产品tag
                        string sql4 = @"SELECT * FROM [shop2.haiwangcn.cn].[dbo].[Hishop_ProductTag] WHERE ProductId =@ProductID";
                ...
                string sqlCheck = @" select * FROM [shop2.haiwangcn.cn].[dbo].[Hishop_OrderAlreadySend] where OrderID=@OrderID";
